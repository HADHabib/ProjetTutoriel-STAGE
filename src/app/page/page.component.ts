...
export class PageComponent implements OnInit {
  ...
  constructor(private WeatherService:WeatherService, private storageService : DataStorageService){
    if(!this.storageService.getUserSettings().length){
      this.expectedEntries=cityList.length;
    }else{
      this.expectedEntries=cityList.length+this.storageService.getUserSettings().length;
  ...
  getWeatherData(nameCity : string){
    this.WeatherService.getWeatherData(nameCity).subscribe(data =>{if(data!=undefined){ this.setWeatherData(data);} }, err=>console.error(err));
  ...
  refreshSearch(){
    ...
    if(this.sort==="ascName"){
      ...
      this.dataSearch.sort((a,b)=>
        Number(b.temp) -Number(a.temp)
      )
    }else if(this.sort==="ascFeels"){
      this.dataSearch.sort((a,b)=>
      Number(a.feelslike) -Number(b.feelslike)
    )
    }else if(this.sort==="descFeels"){
      this.dataSearch.sort((a,b)=>
      Number(b.feelslike) -Number(a.feelslike)
    )
    ...
    this.updateListPage();
    this.dataShow=[];
    for(let i=(this.pages-1)*this.numberEntries;i<(this.pages*this.numberEntries<= this.dataSearch.length ? this.pages*this.numberEntries : this.dataSearch.length);i++ ){
