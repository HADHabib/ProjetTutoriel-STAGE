...
export class WeatherComponent {
  ...
  constructor(
    ...
    this.clickEventsubscription = this.appService
      .getClickEvent()
      .subscribe(() => {
        this.newWeatherData();
      });
  ...
  getWeatherData(nameCity: string, color: string) {
    this.WeatherService.getWeatherData(nameCity).subscribe((data) => {
      if (data != undefined) {
        this.setWeatherData(data, color);
      }
    });
  }
  ...
  setWeatherData(data: any, color: string) {
    ...
    this.WeatherData[index].isDay =
      currentDate.getTime() < sunsetTime.getTime();
    this.WeatherData[index].temp_celcius = (
      this.WeatherData[index].main.temp - 273.15
    ).toFixed(0);
    this.WeatherData[index].temp_min = (
      this.WeatherData[index].main.temp_min - 273.15
    ).toFixed(0);
    this.WeatherData[index].temp_max = (
      this.WeatherData[index].main.temp_max - 273.15
    ).toFixed(0);
    this.WeatherData[index].temp_feels_like = (
      this.WeatherData[index].main.feels_like - 273.15
    ).toFixed(0);
